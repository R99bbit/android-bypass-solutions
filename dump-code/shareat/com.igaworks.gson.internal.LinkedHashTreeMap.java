...
public final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        public Iterator<Entry<K, V>> iterator() {
            return new LinkedTreeMapIterator<Entry<K, V>>(LinkedHashTreeMap.this) {
    ...
    class KeySet extends AbstractSet<K> {
        ...
        public Iterator<K> iterator() {
            return new LinkedTreeMapIterator<K>(LinkedHashTreeMap.this) {
                public K next() {
                    return nextNode().key;
                }
            };
    ...
    public Node<K, V> find(K key, boolean create) {
        ...
        Node<K, V> nearest = nodeArr[index];
        int comparison = 0;
        if (nearest != null) {
        ...
        if (nearest != null) {
            ...
            created = new Node<>(nearest, key, hash, header2, header2.prev);
            nodeArr[index] = created;
        } else {
            throw new ClassCastException(new StringBuilder(String.valueOf(key.getClass().getName())).append(" is not Comparable").toString());
        }
        int i = this.size;
        this.size = i + 1;
        if (i > this.threshold) {
