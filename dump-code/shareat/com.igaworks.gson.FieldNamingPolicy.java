...
public enum FieldNamingPolicy implements FieldNamingStrategy {
    ...
    LOWER_CASE_WITH_UNDERSCORES {
        public String translateName(Field f) {
            return FieldNamingPolicy.separateCamelCase(f.getName(), EventsFilesManager.ROLL_OVER_FILE_NAME_SEPARATOR).toLowerCase();
        }
    },
    ...
    private static String modifyString(char firstCharacter, String srcString, int indexOfSubstring) {
        if (indexOfSubstring < srcString.length()) {
            return new StringBuilder(String.valueOf(firstCharacter)).append(srcString.substring(indexOfSubstring)).toString();
        }
